...
SELECT OPTION_NM FROM OPTION_TB
JOIN OPTION_TYPE USING(OPTION_CD)
WHERE P_ORDER_CD = 61;
...
INSERT INTO REVIEW
		...
		#{starRating},  #{reviewCode}, #{memberNo}, #{productCode}, 
		${productOrderCode}, #{subOrderCode})

-- INSERT INTO REVIEW VALUES(
-- 	SEQ_REVIEW_NO.NEXTVAL, '두구두구<br>', SYSDATE, 10, 1, 7, NULL, NULL, 1
-- )

;

SELECT P_ORDER_CD, P_ORDER_DT, PRODUCT_NM,OPTION_NM, TOTAL_PRICE,REVIEW_CD, PRODUCT.P_CD FROM PRODUCT_ORDER
LEFT JOIN REVIEW USING(P_ORDER_CD)
JOIN OPTION_TB USING(P_ORDER_CD)
JOIN OPTION_TYPE USING(OPTION_CD)
JOIN PRODUCT ON(OPTION_TYPE.P_CD = PRODUCT.P_CD)
...
SET S_REVIEW_ST = 'Y'
WHERE S_ORDER_CD = 
		(SELECT SUBS_ORDER.S_ORDER_CD
		FROM SUBS_ORDER 
		JOIN REVIEW ON(SUBS_ORDER.S_ORDER_CD = REVIEW.S_ORDER_CD)
		WHERE SUBS_ORDER.S_ORDER_CD = REVIEW.${subOrderCode});
...
SELECT SUBS_ORDER.S_ORDER_CD, REVIEW.S_ORDER_CD
FROM SUBS_ORDER 
JOIN REVIEW ON(SUBS_ORDER.S_ORDER_CD = REVIEW.S_ORDER_CD)
WHERE SUBS_ORDER.S_ORDER_CD = REVIEW.S_ORDER_CD;

SELECT PRODUCT_ORDER.P_ORDER_CD, REVIEW.P_ORDER_CD
FROM PRODUCT_ORDER 
JOIN REVIEW ON(PRODUCT_ORDER.P_ORDER_CD = REVIEW.P_ORDER_CD)
WHERE PRODUCT_ORDER.P_ORDER_CD = REVIEW.P_ORDER_CD;
...
-- 리뷰 상품 정보 불러오는
SELECT S_DT, S_NM, S_PRICE, REVIEW_CD FROM SUBS_ORDER
LEFT JOIN REVIEW USING(S_ORDER_CD)
JOIN SUBS USING(S_CD)
WHERE SUBS_ORDER.MEMBER_NO = 7;
...
SELECT REVIEW_NO, MEMBER_NAME, STAR_RATING, REVIEW_CONTENT, REVIEW_CD, 
...
-- 리뷰 구독박스 조회
SELECT REVIEW_NO, MEMBER_NAME, REVIEW_ENROLL_DT, STAR_RATING, REVIEW_CONTENT, REVIEW_CD
FROM REVIEW
JOIN MEMBER USING(MEMBER_NO);
