...
SELECT    '3', 'J20220719-00129' FROM DUAL);
 ...
 FROM SUBS_EXCEPTION WHERE P_CD IN (1,2,3);
...
INSERT INTO MEMBER VALUES(SEQ_MEMBER_NO.NEXTVAL, '박예진', 'gg1221@gmail.com', 'N', DEFAULT, DEFAULT); --2
INSERT INTO MEMBER VALUES(SEQ_MEMBER_NO.NEXTVAL, '이마크', 'markLee@gmail.com', 'N', DEFAULT, DEFAULT); --3


-- 구독 취소상태 -> 구독중으로 update
UPDATE SUBS_ORDER SET S_CANCEL='N'
WHERE S_CANCEL='Y';
...
-- 스케쥴러 확인용
SELECT S_DELIVERY_CD, DELIVERY_CD, S_CYCLE FROM SUBS_DELIVERY JOIN SUBS_ORDER USING (S_ORDER_CD);
-- 상태 전부 배송전으로 변경하기
UPDATE SUBS_DELIVERY SET DELIVERY_CD='1';
-- 샘플데이터 추가
...
SELECT 
   CASE
      WHEN (TO_CHAR (SYSDATE + 1, 'fmiw') - TO_CHAR (TRUNC (SYSDATE, 'mm') + 1, 'fmiw') + 1 < 3) || (SUBSTR(TO_CHAR(SYSDATE, 'YYMMDD'), 5,6) < 10) THEN NEXT_DAY(SYSDATE, 'WED')
      ELSE  NEXT_DAY(SYSDATE +7, 'WED')
   END
FROM DUAL;

TO_DATE('20220718', 'YYMMDD')
...
-- 해당월의 몇주차인지 구하는 구문
SELECT   TO_CHAR (TO_DATE('20220707'-1, 'YYMMDD') + 1, 'fmiw')
       - TO_CHAR (TRUNC (TO_DATE('20220707'-1, 'YYMMDD'), 'mm') + 1, 'fmiw')  + 1
FROM DUAL ; -- 계산한 값이 1이거나 2이고, 
...
SELECT
   CASE WHEN (TO_CHAR (TO_DATE('20220705', 'YYMMDD') + 1, 'fmiw')
       - TO_CHAR (TRUNC (TO_DATE('20220705', 'YYMMDD'), 'mm') + 1, 'fmiw')  + 1) < 3 AND --2
      (TRUNC(TO_DATE('20220705', 'YYMMDD'), 'IW')+2) < TO_DATE('20220707', 'YYMMDD') THEN '첫째 수요일 전'
      ELSE '첫째 수요일 이후'
      END 격주
FROM DUAL;
...
-- 일자만 나오게 자르기
SELECT SUBSTR(TO_CHAR(SYSDATE, 'YYMMDD'), 5,6) FROM DUAL;




-- 해당 주의 수요일 구하는 방법
SELECT TRUNC(TO_DATE('20220707'-1, 'YYMMDD') , 'IW')+2 FROM DUAL;
SELECT TRUNC(SYSDATE, 'IW')+2 FROM DUAL;


-- 첫째주 수요일 구하는 방법
SELECT NEXT_DAY((TRUNC(SYSDATE,'MM')),'wed') FROM DUAL;
...
SELECT PRODUCT_NM, FARM_NO FROM PRODUCT;
SELECT * FROM OPTION_TYPE WHERE P_CD='5';
...
FROM PRODUCT;
		...
		SELECT P_ORDER_CD,TOTAL_PRICE,DELIVERY_ST,
			...
			, '[원산지 ' || (SELECT ORIGIN || '] ' ||	PRODUCT_NM 
							...
							WHERE ROWNUM = 1
                     AND C.P_ORDER_CD = '20220723-0040') ORIGIN,
		...
		WHERE MEMBER_NO = '5'
      AND A.P_ORDER_CD = '20220723-0040'
	ORDER BY P_ORDER_DT DESC;



SELECT PRODUCT_NM, B.P_ORDER_CD
FROM PRODUCT_ORDER B
JOIN OPTION_TB ON(B.P_ORDER_CD = OPTION_TB.P_ORDER_CD)
JOIN OPTION_TYPE USING(OPTION_CD)
JOIN PRODUCT USING(P_CD)
...
WHERE MEMBER_NO='5'
AND ROWNUM=1
AND B.P_ORDER_CD = '20220723-0040';
...
-- 스케줄러 테스트용
SELECT S_ORDER_CD, DELIVERY_ST, S_CANCEL, S_CYCLE
FROM SUBS_DELIVERY
JOIN PRODUCT_DELIVERY USING(DELIVERY_CD)
...
SELECT * FROM SUBS_ORDER
WHERE S_ORDER_CD='S20220726-00153';

SELECT * FROM SUBS_DELIVERY
WHERE S_ORDER_CD='S20220726-00153';

UPDATE SUBS_DELIVERY SET DELIVERY_CD='3'
WHERE S_ORDER_CD='S20220726-00153';
...
SELECT * FROM REVIEW WHERE MEMBER_NO = 7;
SELECT * FROM PRODUCT_ORDER WHERE MEMBER_NO = 7;
