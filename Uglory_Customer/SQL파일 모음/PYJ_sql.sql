...
-- *** 주문 내역 상세 조회 *** 
SELECT P_ORDER_CD, PRODUCT_NM, PRODUCT_PRICE, OPTION_PRICE, OPTION_NM, DELIVERY_ST, OPTION_COUNT
FROM PRODUCT_ORDER
JOIN OPTION_TB USING(P_ORDER_CD)
JOIN OPTION_TYPE USING(OPTION_CD)
JOIN PRODUCT USING(P_CD)
...
-- *** 주문 정보 조회 ***
SELECT MEMBER_NAME,P_PAY_DT,TOTAL_PRICE,  P_ORDER_NM,P_ORDER_PHONE,P_ORDER_ADDR,P_DL_REQ
FROM PRODUCT_ORDER 
JOIN MEMBER USING(MEMBER_NO)
...
--구독 상품 주문 조회
SELECT S_ORDER_CD,S_NM,S_PAY_AMOUNT,
FROM SUBS_ORDER
JOIN SUBS USING(S_CD)
JOIN SUBS_PAY(S_ORDER_CD);
