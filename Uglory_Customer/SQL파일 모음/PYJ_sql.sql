...
-- *** 구독 내역 목록 ***
SELECT S_ORDER_CD,S_NM,S_PRICE,DELIVERY_ST,
	   TO_CHAR(S_DT,'YYYY/MM/DD') S_DT
FROM SUBS_ORDER
JOIN SUBS USING(S_CD)
JOIN SUBS_DELIVERY USING(S_ORDER_CD)
JOIN PRODUCT_DELIVERY USING(DELIVERY_CD)
...
-- *** 구독 정보 조회 ***
SELECT S_ORDER_CD,MEMBER_NAME,S_DT,S_PRICE,S_ORDER_NM,S_ORDER_PHONE,S_ORDER_ADDR,S_DL_REQ,
	(SELECT LISTAGG(PRODUCT_NM, ',') WITHIN GROUP (ORDER BY S_ORDER_CD)
	FROM SUBS_ORDER 
	JOIN SUBS_EXCEPTION USING(S_ORDER_CD)
	JOIN PRODUCT USING(P_CD)
	WHERE S_ORDER_CD = 2) PRODUCT_NM
FROM SUBS_ORDER
JOIN MEMBER USING(MEMBER_NO)
JOIN SUBS USING(S_CD);
